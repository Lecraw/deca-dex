...
export async function POST(req: NextRequest) {
  ...
  if (action === "end_session") {
    ...
    const event = await prisma.decaEvent.findUnique({
      where: { code: roleplaySession.eventCode },
    });
  ...
  if (action === "submit_quiz") {
    ...
    if (!sessionId || !Array.isArray(answers)) {
      return new Response(
        JSON.stringify({ error: "sessionId and answers[] required" }),
        { status: 400 }
      );
    }
    ...
    if (!cached || !Array.isArray(cached) || cached.length === 0) {
      return new Response(JSON.stringify({ error: "No quiz available." }), { status: 400 });
    }
